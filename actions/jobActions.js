...
export const postJob = (data) => {
  return async (dispatch, getstate) => {
    //  console.log("value of getstate", getstate());
    const userId = getstate().auth.userId;
    //  console.log("üser id", userId);
    const rules = {
...
/*  console.log(
      "efwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww",
      data.description,
      "scac",
      data.phone,
      userId
...
/*
    ...
    try {
      ...
      const response = await fetch(
        `https://findjob-f245c.firebaseio.com/allJobs.json?auth=${token}`,
        {
          method: "POST",
          headers: {
      ...
      console.log("eee", formattedErrors);
      dispatch({
...
export const fetchAllJobs = () => {
  return async (dispatch, getState) => {
    ...
    try {
      const response = await fetch(
        `https://findjob-f245c.firebaseio.com/allJobs.json`
      );
      ...
      for (const key in resData) {
        ...
        console.log(resData[key].bgColor);
      }
...
export const deleteJob = (jobId) => {
  return async (dispatch) => {
    ...
    await fetch(
      `https://findjob-f245c.firebaseio.com/allJobs/${jobId}.json?auth=${token}`,
      {
        method: "DELETE",
      }
    );
